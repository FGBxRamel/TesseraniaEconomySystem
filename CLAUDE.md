...
- **Commands**: each subsystem is its own top-level Brigadier command (`/shop`, `/rechnung`, `/spieler`, ...) rather than nested under a shared `/tes` root — implement each as a real Brigadier tree (`Commands.literal(...).then(...)`), registered via `TesBootstrap` (a `PluginBootstrap`, using `LifecycleEvents.COMMANDS`) during the bootstrap phase — not Paper's `BasicCommand` shim, and not YAML-declared commands (Paper plugins don't support those). `BasicCommand` shipped in Stage 0 first and had broken tab-completion; follow the shape already established in `ShopCommand`/`SpielerCommand` for new subcommands, with per-action permission checks via `.requires(...)`.
...
- **Belohnungsinventar** ("reward inventory") — whenever a player earns rewards from the TP or level systems, items land in a virtual inventory opened via `/belohnung`, never inside a live shop chest. Purchases in shops do NOT go through the reward inventory.
...
- A player must first be registered via `/spieler add <Name>` before the reward system is active for them; unregistered players can still use shops/invoices but earn nothing. `/spieler remove` requires re-confirmation and permanently deletes all of that player's system data.
- **Pause** (`/spieler pause|unpause <Name>`) is a sanction mechanism, distinct from being unregistered: a paused player can't buy in item shops, can't sell through their own shop (once every owner of a shop is paused, it stops selling to anyone), can't withdraw earned shop diamonds, can't edit their shops, and can't create new invoices or cash out settled ones — but paying an invoice they owe and retracting one they sent must always stay possible regardless of pause state (§1.4). Apply this same distinction (registered vs. paused, and what pause blocks vs. always permits) when gating new Stage 3+ features like the loyalty shop.

### Transaction capture (3.1)

Three transaction kinds, all of which feed the reward systems:
1. **Item shops** (default, no redstone) — modified vanilla containers (chest, double chest, "redstone chest"/hopper-adjacent variants, barrel, shulker box + colors) turned into sell points. Required attributes: ID, name, owner(s), position, item (may be the special value "all", selling any item except diamonds), price-per-slot (1-64 diamonds). Optional: teleport point. Managed via `/shop erstellen|bearbeiten|schließen|liste [id]` (no `<world>` argument — shop IDs are globally unique, and the spec dropped the per-world argument as of v1.3), modeled UX-wise after the BlueMap Marker plugin's chat-based setup flow. Purchases are refundable for 60 seconds (cooldown-overlay UX like an ender pearl cooldown) before counting as a completed transaction; only the shop owner can withdraw earned diamonds (after the cooldown) or restock the sold item.
...
3. **Dienstleistungen / Trödelmarkt** (services / flea market) — modeled as invoices: `/rechnung erstellen <Ziel> <Preis> <Grund>` (price capped at 2304 Taler, creator must be registered) notifies the target on next login/activity; `/rechnung anzeigen` opens an interface listing open invoices, lets the payer settle by clicking (transfers diamonds to the invoice creator's virtual balance, with a short notification back to the creator), and lets the creator cash out that balance (payout lands in the reward inventory, not directly in their real inventory). The creator can also retract a still-open invoice they sent via a "Versendete Rechnungen" sub-interface (reachable from the open-invoices list), notifying both parties.

### Loyalty points (3.2) — `/punkte` or `/treuepunkte`
